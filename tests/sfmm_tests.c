...
Test(my_test_1, check_if_list_empty, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;


	sf_malloc(sizeof(int));

	int empty_bool = empty_list(sf_free_list_heads[0]);

	cr_assert(empty_bool == 1, "first list should be empty.");
}

Test(my_test_2, check_if_list__not_empty, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;


	sf_malloc(sizeof(int));

	int empty_bool = empty_list(sf_free_list_heads[7]);

	cr_assert(empty_bool == 0, "second to last list should not be empty.");
}

Test(my_test_3, check_if_list__not_empty, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;


	sf_malloc(sizeof(int));

	size_t x = max_mem_of_list(sf_free_list_heads[7]);
	cr_assert(x == 4016, "max block in second to last list should equal 4016");
}

Test(my_test_4, find_right_free_list, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	...
	sf_block* r_ptr = &r;
	r_ptr->header = 0x20;
	sf_block* test = find_smallest_list(r_ptr);
	cr_assert(test == &sf_free_list_heads[0], "smallest list needed should be the first list");
}

Test(my_test_5, malloc_a_negative_value, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;

	int *u = sf_malloc(-1);

	cr_assert(u == NULL, "since malloc of -1 should not be possible, return NULL");
